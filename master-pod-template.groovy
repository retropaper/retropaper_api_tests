...
def output(String stage, String status) {
	...
	try {
		...
		if (status == 'failure'){
			slackMessage = "retropaper-api-tests Master: Build ${env.BUILD_ID} ${status} during ${stage}. See ${env.BUILD_URL}"
			//makeIssue(stage)
		}
		else {
			slackMessage = "retropaper-api-tests Master: Build ${env.BUILD_ID} ${status} during ${stage}."
		}
...
kind: Pod
metadata:
  name: retropaper-api-tests-master-build
spec:
  containers:
  - name: kaniko
